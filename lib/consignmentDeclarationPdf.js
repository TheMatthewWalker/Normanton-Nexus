...
export async function buildConsignmentDeclarationPdf(data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      // ── Matrix table — one row per delivery line this declaration draws
      // down, grouped implicitly by material since the caller already sorts
      // lines by Material then ExpiryDate. ─────────────────────────────────
      const cols = [
        { key: 'material', label: 'Material', w: 80 },
        { key: 'invoice',  label: 'Invoice / Ref', w: 110 },
        { key: 'matdoc',   label: 'GR Doc', w: 80 },
        { key: 'expiry',   label: 'Expiry', w: 70 },
        { key: 'qty',      label: 'Qty Declared', w: CW - (80 + 110 + 80 + 70), align: 'right' },
      ];
