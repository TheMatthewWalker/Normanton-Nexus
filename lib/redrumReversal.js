...
import { sapConfig, sapServerSecret, getProductionPool, sqlConfig } from '../config.js';
...
export async function maybeReverseBatchManagedReturn({
  ...
  try {
    ...
    if (postingRow.recordset.length) {
      ...
      // dbo.AgreementSnapshot lives on a different database to prod.* — a
      // separate connection via sqlConfig, same as the original increment.
