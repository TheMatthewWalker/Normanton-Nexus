 ...
 *
 * Persists sessions to the Nexus database (see config.js's getNexusPool()),
 * reusing the exact same connection pattern as the DB layer files under
 * routes/ — a cached pool per process. No new dependency or infrastructure
 * required.
 *
 * Table: dbo.PortalSessions — see sql/migrate_portal_sessions.sql.
...
export class SqlSessionStore extends Store {
  ...
  set(sid, sessionData, callback) {
    ...
    this._pool()
      .then(pool => pool.request()
        ...
        .query(`
          IF EXISTS (SELECT 1 FROM dbo.PortalSessions WHERE SessionID = @sid)
  ...
  destroy(sid, callback) {
    this._pool()
      .then(pool => pool.request()
        .input('sid', sql.NVarChar(128), sid)
        .query(`DELETE FROM dbo.PortalSessions WHERE SessionID = @sid`))
      .then(() => callback(null))
      .catch(err => {
  ...
  async cleanupExpired() {
    ...
    const result = await pool.request().query(`
      DELETE FROM dbo.PortalSessions WHERE ExpiresUtc <= GETUTCDATE()
    `);
