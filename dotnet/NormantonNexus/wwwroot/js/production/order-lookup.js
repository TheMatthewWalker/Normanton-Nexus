...
(function () {
  ...
  async function search() {
    ...
    try {
      ...
      resultsEl.innerHTML = `<div class="nx-empty">${esc(err.message)}</div>`;
    }
  }

  function render(rows) {
    const el = document.getElementById("ol-results");
    el.innerHTML = rows.length === 0 ? `<div class="nx-empty">No open order lines found.</div>` : `
      <div class="nx-toolbar" style="margin-bottom:8px;">
        ...
        <tbody>
          ${rows.map((r) => `
            <tr>
              ...
              <td><span class="badge badge--accent">${esc(r.valueStream)}</span></td><td>${fmtDate(r.requestDate)}</td>
