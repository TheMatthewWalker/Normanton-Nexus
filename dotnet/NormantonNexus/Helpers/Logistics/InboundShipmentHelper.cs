...
internal static class InboundShipmentHelper
{
    ...
    /// <summary>Writes a freshly-built PO PDF to GetPoPdfPath's location, creating the vendor subfolder if needed — Sub-phase 8b.7's Create PO in SAP / Recreate PO PDF.</summary>
    internal static async Task<string> SavePoPdfAsync(string? vendorName, string poNumber, byte[] pdfBytes, LogisticsOptions settings, CancellationToken ct)
    {
        var filePath = GetPoPdfPath(vendorName, poNumber, settings);
