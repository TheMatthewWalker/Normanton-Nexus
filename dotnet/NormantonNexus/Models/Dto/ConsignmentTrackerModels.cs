...
public sealed record ConsignmentVendorRow(
    int VendorId, string VendorName, string? SapVendorNumber, string? Currency,
...
public sealed record VendorMaterialRow(int VendorMaterialId, string Material, string? ScheduleAgreement);
...
public sealed record ConsignmentDeliveryRow(
    int DeliveryId, int VendorId, string Material, string MaterialDocument, string MaterialDocItem,
...
public sealed record ReversalCancellationZeroedRow(int DeliveryId, string Material, string MaterialDocument, string MaterialDocItem, decimal Quantity);

public sealed record ReversalCancellationReviewRow(int DeliveryId, string Material, string MaterialDocument, string MaterialDocItem, decimal Quantity, decimal RemainingQty, string Reason);
...
public sealed record AllocationProposalLine(
    int DeliveryId, string Material, decimal QtyAllocated, string? InvoiceNumber,
...
public sealed record CreateDeclarationLineRequest(int DeliveryId, string Material, decimal QtyAllocated);
...
public sealed record ConsignmentDeclarationLineRow(
    int DeclarationLineId, int DeliveryId, string Material, decimal QtyAllocated,
...
public sealed record ConsignmentDeclarationSummaryRow(
    int DeclarationId, int VendorId, string VendorName, string Status, string AllocationMethod, decimal TotalQty,
...
public sealed record CancelledDeclarationLine(int DeclarationLineId, int DeclarationId, int CancelledDeliveryId, string Material, decimal QtyAllocated);

public sealed record OpenDeliveryForReassignment(int DeliveryId, string Material, decimal RemainingQty, DateTime? ExpiryDate, DateTime? DocumentDate);

public sealed record ReassignmentSplit(int DeliveryId, decimal Qty);

public sealed record ReassignmentPlanItem(int DeclarationLineId, int DeclarationId, string Material, int CancelledDeliveryId, decimal TotalQty, IReadOnlyList<ReassignmentSplit> Splits, decimal Shortfall);
