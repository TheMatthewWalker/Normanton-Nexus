/* ============================================================
   Extrusion test-data cleanup - EXECUTE
   Database: Production   Process: EX (prod.Extrusion, PK ExtrusionID)
   SQL Server 2005 compatible: no inline DECLARE-init, no CONCAT().

   HOW TO USE
   1. Run preview_extrusion.sql first and sanity-check the counts.
   2. Set @Cutoff below (first REAL ExtrusionID), leave @Commit = 0, and
...
IF @Cutoff IS NULL
BEGIN
    RAISERROR(N'Set @Cutoff to the first real ExtrusionID before running this script.', 16, 1);
    RETURN;
END
...
WHERE (ChildProcessCode  = 'EX' AND ChildRecordID  < @Cutoff)
   OR (ParentProcessCode = 'EX' AND ParentRecordID < @Cutoff);
SET @n = @@ROWCOUNT; PRINT N'prod.ProductionTrace deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.BatchOperators WHERE ProcessCode = 'EX' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.BatchOperators deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.ScrapEntries WHERE ProcessCode = 'EX' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.ScrapEntries deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.SAPPostings WHERE ProcessCode = 'EX' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.SAPPostings deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.EventLog WHERE ProcessCode = 'EX' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.EventLog deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.Extrusion WHERE ExtrusionID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.Extrusion deleted: ' + CAST(@n AS NVARCHAR(10));
