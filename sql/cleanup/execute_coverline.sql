/* ============================================================
   Coverline test-data cleanup - EXECUTE
   Database: Production   Process: CL (prod.Coverline, PK CoverlineID)
   SQL Server 2005 compatible: no inline DECLARE-init, no CONCAT().

   HOW TO USE
   1. Run preview_coverline.sql first and sanity-check the counts.
   2. Set @Cutoff below (first REAL CoverlineID), leave @Commit = 0, and
...
IF @Cutoff IS NULL
BEGIN
    RAISERROR(N'Set @Cutoff to the first real CoverlineID before running this script.', 16, 1);
    RETURN;
END
...
WHERE (ChildProcessCode  = 'CL' AND ChildRecordID  < @Cutoff)
   OR (ParentProcessCode = 'CL' AND ParentRecordID < @Cutoff);
SET @n = @@ROWCOUNT; PRINT N'prod.ProductionTrace deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.BatchOperators WHERE ProcessCode = 'CL' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.BatchOperators deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.ScrapEntries WHERE ProcessCode = 'CL' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.ScrapEntries deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.SAPPostings WHERE ProcessCode = 'CL' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.SAPPostings deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.EventLog WHERE ProcessCode = 'CL' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.EventLog deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.Coverline WHERE CoverlineID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.Coverline deleted: ' + CAST(@n AS NVARCHAR(10));
