/* ============================================================
   Firewall test-data cleanup - EXECUTE
   Database: Production   Process: FW (prod.Firewall, PK FirewallID)
   SQL Server 2005 compatible: no inline DECLARE-init, no CONCAT().

   Standalone cleanup by Firewall's OWN ID range - see preview_firewall.sql
   header for when to use this vs. execute_ewald.sql.

   HOW TO USE
   1. Run preview_firewall.sql first and sanity-check the counts.
   2. Set @Cutoff below (first REAL FirewallID), leave @Commit = 0, and
...
IF @Cutoff IS NULL
BEGIN
    RAISERROR(N'Set @Cutoff to the first real FirewallID before running this script.', 16, 1);
    RETURN;
END
...
WHERE (ChildProcessCode  = 'FW' AND ChildRecordID  < @Cutoff)
   OR (ParentProcessCode = 'FW' AND ParentRecordID < @Cutoff);
SET @n = @@ROWCOUNT; PRINT N'prod.ProductionTrace deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.BatchOperators WHERE ProcessCode = 'FW' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.BatchOperators deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.ScrapEntries WHERE ProcessCode = 'FW' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.ScrapEntries deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.SAPPostings WHERE ProcessCode = 'FW' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.SAPPostings deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.EventLog WHERE ProcessCode = 'FW' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.EventLog deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.Firewall WHERE FirewallID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.Firewall deleted: ' + CAST(@n AS NVARCHAR(10));
