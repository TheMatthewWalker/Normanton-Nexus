/* ============================================================
   Convoluting test-data cleanup - EXECUTE
   ...
   HOW TO USE
   1. Run preview_convoluting.sql first and sanity-check the counts.
   2. Set @Cutoff below (first REAL ConvolutingID), leave @Commit = 0, and
...
IF @Cutoff IS NULL
BEGIN
    RAISERROR(N'Set @Cutoff to the first real ConvolutingID before running this script.', 16, 1);
    RETURN;
END
...
WHERE (ChildProcessCode  = 'CO' AND ChildRecordID  < @Cutoff)
   OR (ParentProcessCode = 'CO' AND ParentRecordID < @Cutoff);
SET @n = @@ROWCOUNT; PRINT N'prod.ProductionTrace deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.BatchOperators WHERE ProcessCode = 'CO' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.BatchOperators deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.ScrapEntries WHERE ProcessCode = 'CO' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.ScrapEntries deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.SAPPostings WHERE ProcessCode = 'CO' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.SAPPostings deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.EventLog WHERE ProcessCode = 'CO' AND ProcessRecordID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.EventLog deleted: ' + CAST(@n AS NVARCHAR(10));

DELETE FROM prod.Convoluting WHERE ConvolutingID < @Cutoff;
SET @n = @@ROWCOUNT; PRINT N'prod.Convoluting deleted: ' + CAST(@n AS NVARCHAR(10));
