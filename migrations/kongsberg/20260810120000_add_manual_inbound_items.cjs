...
exports.up = async function (knex) {
  await knex.raw(`
    IF NOT EXISTS (
      ...
      WHERE object_id = OBJECT_ID(N'dbo.ManualInboundItem') AND type = 'U'
    )
    BEGIN
      CREATE TABLE dbo.ManualInboundItem (
        ...
        CONSTRAINT FK_ManualInboundItem_Shipment FOREIGN KEY (ShipmentId) REFERENCES dbo.PurchaseOrderShipment (ShipmentId)
      );

      CREATE INDEX IX_ManualInboundItem_Shipment ON dbo.ManualInboundItem (ShipmentId);
    END
  `);
};
...
exports.down = async function (knex) {
  await knex.raw(`
    IF EXISTS (
      ...
      DROP TABLE dbo.ManualInboundItem
  `);
};
