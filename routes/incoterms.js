...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            .query('SELECT * FROM log.Incoterms');
...
router.get('/id/:incotermsId', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query('SELECT * FROM log.Incoterms WHERE incotermsID = @incotermsId');
        res.json(result.recordset);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
router.post('/', async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`INSERT INTO log.Incoterms (incotermsID, incotermsDescription)
